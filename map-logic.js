...
function getSharpnessColor(val) {
    if (isNaN(val)) return null;
    if (val >= 1.9) return 'rgba(222, 51, 36)';    // #de3324 Red
    if (val >= 1.6) return 'rgba(141, 30, 91)';   // #8d1e5b Purple Red
    if (val >= 1.3) return 'rgba(89, 21, 151)';   // #591597 Purple
    if (val >= 1.0) return 'rgba(52, 21, 204)';   // #3415cc Purple Blue 
    ...
    toggles.forEach(toggle => {
        toggle.addEventListener('change', function() {
            if (this.checked) {
                ...
                toggles.forEach(t => { if (t !== this) t.checked = false; });
            }
            applyHeatmaps();
        });
    });
...
function updateDisplay(row, title) {
    ...
    let detailsHTML = '';
    const fields = [
        { key: 'Sound Pressure Level (dB)', label: 'SPL (dBA)' },
        { key: 'ISOPleasantness', label: 'ISO Pleasantness' },
        { key: 'ISOEventfulness', label: 'ISO Eventfulness' },
        { key: 'Sound Pressure Level (dBC)', label: 'SPL (dBC)' },
        { key: 'Sound Pressure Level (dBZ)', label: 'SPL (dBZ)' },
        { key: 'Loudness (sone)', label: 'Loudness (sone)' },
        { key: 'Sharpness (acum)', label: 'Sharpness (acum)' },
        { key: 'Roughness (asper)', label: 'Roughness (asper)' },
    ...
    fields.forEach(field => {
        ...
        if (val && val.toString().trim() !== '') {
            detailsHTML += `<p><strong>${field.label}:</strong> ${val.trim()}</p>`;
        }
    });
